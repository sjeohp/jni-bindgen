...
fn main() {
    ...
    let directory: &Path = Path::new(matches.value_of("output-path").unwrap_or("../jni-android-sys"));
    ...
    let max_api_level: i32 = matches.value_of("max-api-level").unwrap_or("29").parse().expect("--max-api-level must specify an integer version");
    let android_sdk: Option<&str> = matches.value_of("android-sdk");
